...
async function main() {
  const deployProcess = exec(
    `yarn docs:build && permaweb-deploy --ant-process $DEPLOY_ANT_PROCESS_ID --undername ao-sdk --deploy-folder $DEPLOY_DIR`,
    {
      env: {
        ...process.env,
        DEPLOY_KEY: b64EncodedWallet,
        DEPLOY_ANT_PROCESS_ID: 'wJVTnZTedI9FIY4r2cB9C4CpAJKImvhu0WjOh0AecjQ',
        DEPLOY_DIR: path.join(__dirname, '..', 'docs', '.vitepress', 'dist'),
